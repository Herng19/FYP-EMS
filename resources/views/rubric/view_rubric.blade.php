            ...
            <tbody class="flex flex-col w-full" style="min-height: 70vh;">
                @foreach ($rubric->rubric_criterias as $i => $rubric_criteria)
                    ...
                    @foreach ($rubric_criteria->sub_criterias as $j => $sub_criteria)
                        <tr class="flex mx-8 mt-2 items-center">
                            <td class="py-2 text-gray text-sm font-semibold text-left w-1/4 pl-4 break-all">{{ $i+1 }}.{{ $j+1 }} {{ $sub_criteria->sub_criteria_name }}</td>
                            <td class="py-2 text-gray text-sm font-semibold text-center w-1/4 uppercase text-center break-all">{{ $sub_criteria->co_level }}</td>
                            @foreach ($sub_criteria->criteria_scales as $scale)
                                @if( $scale->scale_level == 2 || $scale->scale_level == 4 )
                                    <td class="py-2 text-gray-400 text-sm font-semibold text-center w-1/4 break-all">{{ $scale->scale_description }}</td>
                                @else
                                    <td class="py-2 text-gray text-sm font-semibold text-center w-1/4 break-all">{{ $scale->scale_description }}</td>
                                @endif
                            @endforeach
                            <td class="py-2 text-gray text-sm font-semibold text-center w-1/4 uppercase text-center break-all">{{ $sub_criteria->weightage }}</td>
                        </tr>
                    @endforeach
                @endforeach
            </tbody>
