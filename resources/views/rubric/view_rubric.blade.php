<x-app-layout>
    ...
    <div class="mb-4 px-8">
        <a href={{ (auth('student')->check())? "/rubric/student-rubric" : "/rubric" }} class="font-bold text-primary-700 text-lg"><i class="fa-regular fa-chevron-left mr-2"></i>{{ $rubric->rubric_name }}</a>
                ...
                @foreach ($rubric->rubric_criterias as $i => $rubric_criteria)
                    ...
                    @foreach ($rubric_criteria->sub_criterias as $j => $sub_criteria)
                        <tr class="flex px-8 pt-2 items-center {{ ($j%2 == 0)? 'bg-primary-50' : '';}}">
                            ...
                            <td class="py-2 text-gray text-xs font-semibold text-center w-1/4 uppercase text-center">{{ $sub_criteria->co_level->co_level_name }}</td>
