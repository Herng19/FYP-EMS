<x-app-layout>
    ...
    @endrole
    <div class="px-8">
        @unlessrole('head of research group')
                ...
                <tbody class="flex flex-col w-full" style="min-height: 70vh;">
                        @foreach ($evaluatees as $i => $evaluatee)
                            <tr class="flex px-8 py-2 {{ ($i%2 == 0)? 'bg-primary-50' : '';}}">
                                ...
                                <td class="py-2 text-gray text-sm font-semibold text-center w-1/4">{{ $evaluatee->psm_year }}</td>
        ...
        @else
            <x-show-table :headers="['Evaluatee Name', 'PSM Year', 'Date', 'Timeslot', 'Venue', 'Evaluator 1', 'Evaluator 2']">
                <tbody class="flex flex-col w-full" style="min-height: 70vh;">
                        @foreach ($evaluatees as $i => $evaluatee)
                            <tr class="flex px-8 py-2 {{ ($i%2 == 0)? 'bg-primary-50' : '';}}">
                                <td class="mx-4 py-2 text-gray text-sm font-semibold w-4">{{ $i+1 }}.</td>
                                ...
                                @elseif($evaluatee->psm_year == '2')
                                    <td class="py-2 text-gray text-sm font-semibold text-left w-1/4">{{ $evaluatee->slot->booths->booth_code }} {{ $evaluatee->slot->booths->booth_name }}</td>
                                @endif
                                <td class="py-2 text-gray text-sm font-semibold text-left w-1/4">{{ $evaluatee->evaluators[0]->name }}</td>
                                <td class="py-2 text-gray text-sm font-semibold text-left w-1/4">{{ $evaluatee->evaluators[1]->name }}</td>
                            </tr>
                        @endforeach
                </tbody>
            </x-show-table>
