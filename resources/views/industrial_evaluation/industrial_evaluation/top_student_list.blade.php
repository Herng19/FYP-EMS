    ...
    <div class="px-8">
        ...
        <x-show-table :headers="['Name', 'PSM', 'Project Title', 'Marks', 'Action']">
            <tbody class="flex flex-col w-full" style="min-height: 70vh;">
                @foreach ($top_students as $i => $evaluatee)
                    <tr class="flex px-8 py-2 items-center {{ ($i%2 == 0)? 'bg-primary-50' : '';}}">
                        ...
                        <td class="py-2 text-gray text-sm font-semibold text-left w-1/4">{{ $evaluatee->project->project_title }}</td>
                        ...
                        <td class="py-2 text-gray text-sm font-semibold text-center w-1/4">
                            <a href="/industrial evaluation/{{ $evaluatee->student_id}}" class="rounded-full px-3 py-3 text-primary-700 hover:bg-primary-100">
