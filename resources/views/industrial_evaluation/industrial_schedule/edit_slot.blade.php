<x-app-layout>
    ...
    <div class="mx-20" id="slot-form">
        <form action="/industrial schedule/edit-slot/{{ $slot->industrial_slot_id }}" method="POST" id="edit-slot-form">
            ...
            <div class="bg-white rounded-md py-6 px-8">
                ...
                <div class="ml-4 mt-2">
                    ...
                    <select id="name" name="name" class="block text-sm font-semibold mt-1 w-full bg-primary-100 text-primary-700 border border-slate-200 focus:ring-primary-400 focus:border-0 px-4 py-2 rounded-md" required>
            ...
            <div class="bg-white rounded-md py-6 px-8">
                ...
                <div class="ml-4 mt-2">
                    {{-- Student's Name --}}
                    <label for="venue" class="text-gray text-xs font-bold">Booth</label>
                    <select id="venue" name="venue" class="block text-sm font-semibold mt-1 w-full bg-white text-gray-700 border border-slate-200 focus:ring-primary-400 focus:border-0 px-4 py-2 rounded-md" required>
                        @foreach($booths as $booth)
                            @if($booth->booth_id == $slot->booth_id)
                                <option value="{{$booth->booth_id}}" class="text-gray-500 text-inherit font-semibold" selected>{{$booth->booth_code}} {{$booth->booth_name}}</option>
                            @else
                                <option value="{{$booth->booth_id}}" class="text-gray-500 text-inherit font-semibold">{{$booth->booth_code}} {{$booth->booth_name}}</option>
                            @endif
                        @endforeach
                    </select>
                    @error('booth')
                    ...
                    <div class="grid grid-cols-2 gap-4 mt-4">
                        ...
                        <div>
                            ...
                            <select id="timeslot" name="timeslot" class="block text-sm font-semibold mt-1 w-full bg-white text-gray-700 border border-slate-200 focus:ring-primary-400 focus:border-0 px-4 py-2 rounded-md" required>
                                @foreach($timeslots as $timeslot)
                                    @if((date("H:i", strtotime($slot->start_time))) == $timeslot)
                                        <option value="{{$timeslot}}" class="text-gray-500 text-inherit font-semibold" selected>{{$timeslot}}</option>
                                    @else
                                        <option value="{{$timeslot}}" class="text-gray-500 text-inherit font-semibold">{{$timeslot}}</option>
                                    @endif
                                @endforeach
                            </select>
                            @error('timeslot')
                    ...
                    <div class="mt-4">
                        ...
                        <select id="evaluator1" name="evaluator1" class="block text-sm font-semibold mt-1 w-full bg-white text-gray-700 border border-slate-200 focus:ring-primary-400 focus:border-0 px-4 py-2 rounded-md" required>
                            @foreach($available_evaluators as $available_evaluator)
                                @if(isset($evaluators[0]) && $available_evaluator->industrial_evaluator_id == $evaluators[0])
                                    <option value="{{$available_evaluator->industrial_evaluator_id}}" class="text-gray-500 text-inherit font-semibold" selected>{{$available_evaluator->evaluator_name}}</option>
                                @else
                                    <option value="{{$available_evaluator->industrial_evaluator_id}}" class="text-gray-500 text-inherit font-semibold">{{$available_evaluator->evaluator_name}}</option>
                                @endif
                            @endforeach
                        </select>
                        @error('evaluator1')
                    ...
                    <div class="mt-4">
                        ...
                        <select id="evaluator2" name="evaluator2" class="block text-sm font-semibold mt-1 w-full bg-white text-gray-700 border border-slate-200 focus:ring-primary-400 focus:border-0 px-4 py-2 rounded-md" required>
                            @foreach($available_evaluators as $available_evaluator)
        ...
        <div class="flex my-8 justify-end items-center">
            ...
            <a href="/industrial schedule"><x-secondary-button class="ml-4" id="cancel-button">Back</x-secondary-button></a>
