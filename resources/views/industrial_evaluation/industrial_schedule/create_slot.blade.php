        ...
        <form action="/industrial schedule/create-slot" method="POST" id="edit-slot-form">
            ...
            <div class="bg-white py-6 px-8 rounded-md">
                ...
                <div class="ml-4 mt-2">
                    ...
                    <select id="name" name="name" class="block text-primary-700 text-sm font-semibold mt-1 w-full bg-primary-100 px-4 py-2 border border-slate-200 focus:ring-primary-400 focus:border-0 rounded-md" required>
                        @foreach($students as $student)
                            @if($student->student_id == $selected_student->student_id)
                                <option value="{{$student->student_id}}" class="font-semibold @if($student->industrial_evaluation_slot !== null) text-primary-200 @else text-primary-700 @endif" selected>{{$student->name}}</option>
                            @else
                                <option value="{{$student->student_id}}" class="font-semibold @if($student->industrial_evaluation_slot !== null) text-primary-200 @else text-primary-700 @endif">{{$student->name}}</option>
                            @endif
                        @endforeach
                    </select>
                    @error('name')
                ...
                <div class="ml-4 mt-2">
                    ...
                    <select id="venue" name="venue" class="block text-sm font-semibold mt-1 w-full bg-white text-gray-700 border border-slate-200 focus:ring-primary-400 focus:border-0 px-4 py-2 rounded-md" required>
                        @foreach($booths as $booth)
                            @if($booth->booth_id == old('booth'))
                                <option value="{{$booth->booth_id}}" class="text-gray-500 font-semibold" selected>{{$booth->booth_code}} {{$booth->booth_name}}</option>
                            @else
                                <option value="{{$booth->booth_id}}" class="text-gray-500 font-semibold">{{$booth->booth_code}} {{$booth->booth_name}}</option>
                            @endif
                        @endforeach
                    </select>
                    @error('venue')
                        ...
                        <select id="evaluator1" name="evaluator1" class="block text-sm font-semibold mt-1 w-full bg-white text-gray-700 border border-slate-200 focus:ring-primary-400 focus:border-0 px-4 py-2 rounded-md" required>
                            @foreach($available_evaluators as $available_evaluator)
                                @if($available_evaluator->industrial_evaluator_id == old('evaluator1'))
                                    <option value="{{$available_evaluator->industrial_evaluator_id}}" class="text-gray-500 font-semibold" selected>{{$available_evaluator->evaluator_name}}</option>
                                @else
                                    <option value="{{$available_evaluator->industrial_evaluator_id}}" class="text-gray-500 font-semibold">{{$available_evaluator->evaluator_name}}</option>
                                @endif
                            @endforeach
                        </select>
                        @error('evaluator1')
