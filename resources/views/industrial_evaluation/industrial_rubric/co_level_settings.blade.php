<x-app-layout>
    ...
    <div class="px-12">
        <div class="flex justify-between">
            <a href="/industrial rubric" class="font-semibold text-primary-700 text-lg"><i class="fa-regular fa-chevron-left mr-2"></i> BACK</a>
            <a href="/industrial rubric/create-co-level"><x-button><i class="fa-regular fa-plus mr-2"></i>New CO Level</x-button></a>
        </div>
        <div class="mt-2">
                    ...
                    @foreach ($co_levels as $i => $co_level)
                        <tr class="flex px-8 py-2 items-center {{ ($i%2 == 0)? 'bg-primary-50' : '';}}">
                            ...
                            <td class="flex py-2 items-center justify-center w-1/3">
                                <a href="/industrial rubric/edit-co-level/{{ $co_level->industrial_co_level_id }}" class="rounded-full py-2 px-3 bg-green-100 border border-green-200 justify-center items-center hover:bg-green-200 ml-2">
                        ...
                        <x-delete-confirmation-modal route="/industrial rubric/delete-co-level/{{ $co_level->industrial_co_level_id }}" title="Delete CO LEVEL" description="Are you sure to delete '{{ $co_level->co_level_name }}' ?" id="{{ $i }}"/>
                    @endforeach
                </tbody>
            </x-show-table>
            <div class="my-4">
