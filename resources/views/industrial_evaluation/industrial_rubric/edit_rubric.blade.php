                                    ...
                                    <div>
                                        <select id="sub-criteria-co-level" name="criteria[{{ $i }}][{{ $j }}][sub_criteria_co_level]" class="block text-sm font-semibold mt-1 w-full bg-white text-gray-700 border border-slate-200 focus:ring-primary-400 focus:border-0 px-4 py-2 rounded-md" required>
                                            ...
                                            @foreach ($co_levels as $co_level)
                                                @if( $co_level->industrial_co_level_id == $sub_criteria->industrial_co_level->industrial_co_level_id )
                                                    <option value="{{ $co_level->industrial_co_level_id }}" class="text-gray-500 font-semibold" selected>{{ $co_level->co_level_name }}</option>
                                                @else
                                                    <option value="{{ $co_level->industrial_co_level_id }}" class="text-gray-500 font-semibold">{{ $co_level->co_level_name }}</option>
                                                @endif
                                            @endforeach
                                        </select>
                                    </div>
